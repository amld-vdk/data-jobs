...
def run(job_input: IJobInput):
    ...
    reviews = job_input.execute_query( # <- !!! BEFORE THE ( ENTER THE APPROPRIATE JOB_INPUT METHOD FOR EXECUTING SQL STATEMENTS FROM PYTHON SCRIPTS !!!
        f"""
        SELECT date, num_no_scent_reviews 
        FROM yankee_candle_reviews_transformed_titi
        """
    )
    reviews_df = pd.DataFrame(reviews, columns=['date', 'num_no_scent_reviews'])

    # Read the covid data and transform to df
    covid = job_input.execute_query( # <- !!! BEFORE THE ( ENTER THE APPROPRIATE JOB_INPUT METHOD FOR EXECUTING SQL STATEMENTS FROM PYTHON SCRIPTS !!!
        f"""
        SELECT * 
        FROM covid_cases_usa_daily_titi
        """
    )
    ...
    # Sort df values by date descending
    df_merged = df_merged.sort_values('date', ascending=False).reset_index(drop=True)
    ...
    df_merged_weekly['date'] = df_merged_weekly['date'].dt.strftime("%Y-%m-%d") # <- !!! ENTER HERE THE PANDAS DATETIME METHOD THAT HANDLES DATETIME FORMAT TRANSFORMATIONS !!!

    # Keep only the non-ingested records
    ...
    if len(df_merged_weekly) > 0:
        ...
        job_input.send_tabular_data_for_ingestion(
            ...
            destination_table="weekly_correlation_titi" # <- !!! ENTER BETWEEN THE QUOTES THE NAME OF THE TABLE WE CREATED IN SCRIPT "04_create_weekly_correlation.sql" !!!
        )
