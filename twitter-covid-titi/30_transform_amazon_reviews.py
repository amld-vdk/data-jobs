...
def run(job_input: IJobInput):
    ...
    props = job_input.get_all_properties() # <- !!! ENTER HERE THE VDK'S JOB_INPUT FUNCTION THAT GETS ALL JOB PROPERTIES !!!
    ...
    reviews_raw = job_input.execute_query(
        f"""
        SELECT *
        FROM yankee_candle_reviews_titi
        WHERE Date > '{props["last_date_amazon_transformed"]}'
        ORDER BY Date
        """
    )
    df = pd.DataFrame(reviews_raw, columns=['date', 'review']) # <- !!! CONVERT THE reviews_raw OBJECT INTO A PANDAS DATAFRAME THROUGH pd.DataFrame(). NAME THE COLUMNS "date" AND "review" !!!
    ...
    if len(df) > 0:
        ...
        job_input.send_tabular_data_for_ingestion(
            ...
            destination_table="yankee_candle_reviews_transformed_titi" # <- !!! ENTER BETWEEN THE QUOTES THE NAME OF THE TABLE WE CREATED IN SCRIPT "03_create_yankee_candle_reviews_transformed.sql" !!!
        )
        # Reset the last_date property value to the latest date in the transformed db table
        props["last_date_amazon_transformed"] = max(df_group['date'])# <- !!! ASSIGN THE MAXIMUM DATE FROM df_group TO THE last_date_amazon_transformed PROPERTY !!!
