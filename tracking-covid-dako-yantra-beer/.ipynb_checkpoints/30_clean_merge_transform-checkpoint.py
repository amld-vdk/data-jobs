...
def run(job_input: IJobInput):
    ...
    cases = job_input.execute_query(
        f"""
        SELECT *
        FROM covid_cases_europe_daily_ddakov_yantra_beer
    ...
    deaths = job_input.execute_query(
        f"""
        SELECT * 
        FROM covid_deaths_europe_daily_ddakov_yantra_beer
        WHERE obs_date > '{props["last_date_cases_deaths"]}'
        """
    )
    ...
    prev_day_df = job_input.execute_query(
        ...
        FROM covid_cases_europe_daily_ddakov_yantra_beer as c
        JOIN covid_deaths_europe_daily_ddakov_yantra_beer as d
    ...
    if len(df_cases_deaths) > 0:
        ...
        job_input.send_tabular_data_for_ingestion(
            ...
            destination_table="covid_cases_deaths_europe_daily_ddakov_yantra_beer"
        )
