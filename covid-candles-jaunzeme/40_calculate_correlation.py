...
def run(job_input: IJobInput):
    ...
    reviews = job_input.execute_query(
        ...
        FROM yankee_candle_reviews_transformed_agita
        """
    )
    ...
    covid = job_input.execute_query( # <- !!! BEFORE THE ( ENTER THE APPROPRIATE JOB_INPUT METHOD FOR EXECUTING SQL STATEMENTS FROM PYTHON SCRIPTS !!!
        f"""
        SELECT * 
        FROM covid_cases_usa_daily_agita
        """
    )
    ...
    if len(df_merged_weekly) > 0:
        ...
        job_input.send_tabular_data_for_ingestion(
            ...
            destination_table="weekly_correlation_agita" # <- !!! ENTER BETWEEN THE QUOTES THE NAME OF THE TABLE WE CREATED IN SCRIPT "04_create_weekly_correlation.sql" !!!
        )
