...
def run(job_input: IJobInput):
    ...
    reviews_raw = job_input.execute_query(
        f"""
        SELECT *
        FROM yankee_candle_reviews_agita
    ...
    if len(df) > 0:
        ...
        job_input.send_tabular_data_for_ingestion(
            ...
            destination_table="yankee_candle_reviews_transformed_agita" # <- !!! ENTER BETWEEN THE QUOTES THE NAME OF THE TABLE WE CREATED IN SCRIPT "03_create_yankee_candle_reviews_transformed.sql" !!!
        )
        # Reset the last_date property value to the latest date in the transformed db table
        props["last_date_amazon_transformed"] = max(df['date']) # <- !!! ASSIGN THE MAXIMUM DATE FROM df_group TO THE last_date_amazon_transformed PROPERTY !!!
